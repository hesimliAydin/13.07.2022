...
int main()
{
	...
	debtors.push_back(Debtor("Robert T. Taylor", DateTime("August", 17, 1932), "270-596-6442", "RobertTTaylor@armyspy.com", "2812 Rowes Lane Paducah, KY 42001", 7785));
		...
		for_each(debtors.begin(), debtors.end(), AgeBetweenUp(20, 40));
		...
		for_each(debtors.begin(), debtors.end(), DebitLessThan5000);
		for_each(debtors.begin(), debtors.end(), DebitLessThan(5000));
	...
	for_each(debtors.begin(), debtors.end(), IfInDecember);
	for_each(debtors.begin(), debtors.end(), WhichMonth("October"));
